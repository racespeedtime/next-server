...
export const useTabsStore = defineStore('tabs', {
  ...
  actions: {
    ...
    async addTab(tab: any) {
      ...
      const hasExist = this.tabList.some(item => item.fullPath === tab.fullPath)
      if (hasExist)
        return
      if (
    ...
    async removeTab(tabPath: string, isCurrent: boolean = true) {
      const index = this.tabList.findIndex(item => item.fullPath === tabPath)
      if (index === -1)
        return
    ...
    // 关闭左边 OR 右边选项卡
    async closeSideTabs(fullPath: string, type: 'left' | 'right') {
      const currentIndex = this.tabList.findIndex(item => item.fullPath === fullPath)
      if (currentIndex !== -1) {
        const range
          = type === 'left'
            ? [0, currentIndex]
    ...
    async closeManyTabs(tabValue?: string) {
      this.tabList = this.tabList.filter((item) => {
        return item.fullPath === tabValue || !item.close
      })
      // 重新设置路由缓存，将新的tabList的name覆盖keepAliveList
      const keepAliveList = this.tabList.filter(item => item.isKeepAlive)
      keepAliveStore.setKeepAliveName(keepAliveList.map(item => item.name))
